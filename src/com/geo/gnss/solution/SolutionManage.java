...
public class SolutionManage {
	...
	public SolutionManage(AppConfig appConfig, SolutionParameter solutionParameter){
		...
		baseStationList = solutionParameter.getBaseStationList();
		roverStationList = solutionParameter.getRoverStationList();
	}
	...
	public boolean parseStatic(){
		...
		String uploadFilePath = solutionParameter.getUploadFilePath();
		if(!uploadFilePath.isEmpty()){
			if(baseFileList == null){
				baseFileList = new ArrayList<>();
			}
			baseFileList.add(uploadFilePath);
		}
	...
	public boolean parseDynamic(){
		...
		String uploadFilePath = solutionParameter.getUploadFilePath();
		if(roverStationList == null || roverStationList.size() == 0){
			return false;
		}
		
		if(baseStationList == null && uploadFilePath.isEmpty() ){
			return false;
		}
	...
	public List<String> parse(List<String> stationList, String folderPath){
		...
		for(String stationName : stationList){
	    	...
	    	MergeDLL64.instance.MergeRawFile(sb.toString(), mergeFilePath);
	    	...
	    	SplitDLL64.instance.Split(mergeFilePath, saveFilePath, splitStartTime, splitEndTime);
    ...
    private boolean readHead(byte[] bt) {
		...
		setStartHour = Integer.parseInt(setStartHourString.split(":")[0]);
		setEndHour = Integer.parseInt(setEndHourString.split(":")[0]);
		...
		int setEndHourMin = Integer.parseInt(setEndHourString.split(":")[1]);
